        ...
        {skillsRows.map((row, i) => (
          <div key={i} className=" flex md:flex-row  justify-center gap-8 ">
            {row.map((skill: { name: string; icon: React.ReactNode }) => (
              ...
              >
                <div className="rounded-full bg-secondary shadow-lg p-5  transition-transform duration-300 group-hover:scale-110 group-hover:shadow-pink-400/40 group-focus:scale-110 group-focus:shadow-pink-400/40 active:scale-110 active:shadow-pink-400/40">
                  {skill.icon}
                </div>
        ...
        {chunkArray(skillsRows.flat(), 4).map((group, i) => (
          <div key={i} className="flex flex-row justify-center gap-6">
            {group.map((skill: { name: string; icon: React.ReactNode }) => (
              ...
              >
                <div className="rounded-full bg-secondary shadow-lg p-4  transition-transform duration-300 group-hover:scale-110 group-hover:shadow-pink-400/40 group-focus:scale-110 group-focus:shadow-pink-400/40 active:scale-110 active:shadow-pink-400/40">
                  {skill.icon}
                </div>
                <span className="mt-2 text-sm  text-gray-300 text-center">
                  {skill.name}
